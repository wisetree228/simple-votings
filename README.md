# Проект "Simple votings"

### Цель
Предоставить пользователю сервис, на котором можно быстро создать голосование и собрать мнения пользователей касательно какого-либо вопроса.

### Технологический стек:
- Python 3.12
- Django 5.1+
- SQLite
### Сам проект на сервере
https://wisetree228.pythonanywhere.com/

### Инструкция по настройке проекта для локального запуска:
1. Склонировать проект
2. Открыть проект в PyCharm с наcтройками по умолчанию
3. Создать виртуальное окружение (через settings -> project "simple votings" -> project interpreter)
4. Открыть терминал в PyCharm, проверить, что виртуальное окружение активировано.
5. Обновить pip:
   ```bash
   pip install --upgrade pip
   ```
6. Установить в виртуальное окружение необходимые пакеты: 
   ```bash
   pip install -r requirements.txt
   ```

7. Создать уникальный ключ приложения.  
   Генерация делается в консоли Python при помощи команд:
   ```bash
   python manage.py shell -c "from django.core.management.utils import get_random_secret_key; print(get_random_secret_key())"
   ```
   Далее полученное значение подставляется в соответствующую переменную.
   Внимание! Без выполнения этого пункта никакие команды далее не запустятся.

7. Синхронизировать структуру базы данных с моделями: 
   ```bash
   python manage.py migrate
   ```
### Описание реализованного функционала
На данный момент реализован весь функционал, кроме смены аватарки и подачи жалоб на посты/комментарии. Можно зарегистрироваться или войти в аккаунт, выйти из аккаунта, создать пост-голосование(сколько угодно вариантов голосования в посте), листать ленту постов и голосовать, ставить лайки, писать и читать комментарии, смотреть результаты голосования в процентах, смотреть проголосовавших за конкретный вариант голосования пользователей, смотреть лайкнутые посты, смотреть свои посты, редактировать данные о себе в профиле (кроме аватарки), в т. ч. менять пароль
   
